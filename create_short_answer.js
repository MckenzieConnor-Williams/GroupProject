(function () {
    ...
    function renderQuestions() {
        questionsTitle.textContent = "Your Short Answers (" + flashcards.length + ")";
        list.innerHTML = "";
    ...
    form.addEventListener("submit", async function (event) {
        ...
        try {
            ...
            if (!response.ok) {
                alert(await readErrorMessage(response, "Failed to save short answer"));
                return;
            }
    ...
    list.addEventListener("click", async function (event) {
        ...
        if (target.classList.contains("delete-btn")) {
            if (!confirm("Delete this short answer?")) return;
            try {
                ...
                if (!response.ok) {
                    alert(await readErrorMessage(response, "Failed to delete short answer"));
                    return;
                }
        ...
        if (target.classList.contains("edit-btn")) {
            ...
            try {
                ...
                if (!response.ok) {
                    alert(await readErrorMessage(response, "Failed to update short answer"));
                    return;
                }
                const data = await response.json();
    ...
    (async function init() {
        ...
        try {
            ...
            console.error(err);
            alert("Unable to load short answers from database.");
            renderQuestions();
        }
    })();
})();
